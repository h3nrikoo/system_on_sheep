...
void logger_log_tag(tag_reading_entry_t * p_reading) {
    m_tag_reading_t * p_queued_reading;

    if (logger.num_tag_readings < LOGGER_MAX_TAG_READINGS) {
        p_queued_reading = &logger.tag_readings[logger.num_tag_readings];
        p_queued_reading->reading_number = logger.total_readings;
        uint32_t gps_delay = app_timer_cnt_diff_compute(app_timer_cnt_get(), logger.gps_time);
        p_queued_reading->gps_delay = gps_delay*1000*(APP_TIMER_CONFIG_RTC_FREQUENCY + 1 ) / APP_TIMER_CLOCK_FREQ; //ms 
        
        uint32_t idx;
        for (idx = 0; idx < MIN(p_reading->packet_count, 64); idx++)
        {
            p_queued_reading->p_samples[idx] = p_reading->p_samples[idx];
        }
        for (; idx < 64; idx++)
        {
            // Set any remaining samples to zero
            p_queued_reading->p_samples[idx] = 0;
        }
